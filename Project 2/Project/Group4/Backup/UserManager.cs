...
namespace Nhom4
{
	...
	private void data()
	{
		...
		private void dbg_List_Click(object sender, System.EventArgs e)
		{
			try
			{
				txt_UserName.Text=dbg_List[dbg_List.CurrentRowIndex,0].ToString().Trim();
				txt_FullName.Text=dbg_List[dbg_List.CurrentRowIndex,1].ToString().Trim();
				txt_Address.Text=dbg_List[dbg_List.CurrentRowIndex,2].ToString().Trim();
				txt_Phone.Text=dbg_List[dbg_List.CurrentRowIndex,3].ToString().Trim();
